...
class TextBlocksWidget extends WP_Widget {
  function TextBlocksWidget() {
    parent::WP_Widget( FALSE, $name = 'Text Blocks Widget' );
  }
  ...
  function form( $instance ) {
    ...
    <p>
      <label
        for="<?php echo $this->get_field_id( 'id' ); ?>"><?php _e( 'Text Block:' ); ?></label>
      <select class="widefat" id="<?php echo $this->get_field_id( 'id' ); ?>"
              name="<?php echo $this->get_field_name( 'id' ); ?>">
        <?php foreach ( $blocks as $block ) { ?>
          <option
            value="<?php echo $block->ID; ?>"<?php if ( $selected_block == $block->ID ) {
    ...
    <p>
      <input id="<?php echo $this->get_field_id( 'wpautop' ); ?>"
             name="<?php echo $this->get_field_name( 'wpautop' ); ?>"
             type="checkbox"<?php if ( $wpautop == "on" ) {
        echo " checked='checked'";
      } ?>>&nbsp;
      <label for="<?php echo $this->get_field_id( 'wpautop' ); ?>">Automatically
...
function text_blocks_shortcode( $atts ) {
  ...
  if ( $id ) {
    return show_text_block( $id, $plain );
  } else {
    return FALSE;
  }
}
